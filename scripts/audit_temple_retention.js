...
(async()=>{
 ...
 const expected=full.players.lompste.data.total_collections_finished;
 ...
 assert.equal(merged.players.lompste.data.total_collections_finished,expected);
 ...
 assert.equal(w.UGTempleStore.merge(full,stale).players.lompste.data.total_collections_finished,expected,'fetch date cannot override newer player data');
