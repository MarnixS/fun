...
(async()=>{
 ...
 for(const page of pages){
  ...
  if(page==='gim.html'){
   ...
   for(const tab of d.querySelectorAll('[data-gim-tab]')){tab.click();await waitFor(()=>!d.querySelector(`[data-gim-panel="${tab.dataset.gimTab}"]`).classList.contains('hidden'),'Collection Log tab '+tab.dataset.gimTab);assert.equal(d.querySelectorAll('[data-gim-panel]:not(.hidden)').length,1);if(tab.dataset.gimTab==='value'){await waitFor(()=>d.querySelector('#clogValue tbody tr'),'valued items');assertLinks(d,'#clogValue .item-cell','Item value')}if(tab.dataset.gimTab==='recent')assertLinks(d,'#recentDrops .drop-card','Recent drops')}
