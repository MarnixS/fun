...
function init(){
 ...
 const nemesis='<button type="button" class="nav-link nav-nemesis-preview" aria-label="Nemesis Comparison, beta" title="Nemesis Comparison · Beta"><span class="nav-rune" aria-hidden="true">VS</span><span>Nemesis Comparison</span><span class="nav-badge">BETA</span></button>';
 nav.innerHTML=brand+link(['index.html','Overview','O'])+group('logs','Collection Log','G',logs)+group('levels','Levels','XP',levels)+group('record','Record','✦',records)+nemesis+`<span class="nav-spacer" aria-hidden="true"></span>`+link(['faq.html','FAQ','?']);
