...
function ranked(metrics){
 const familySizes=lens==='impact'?luckyFamilySizes(metrics):null;
...
function renderLists(){
 const ps=selectedPlayers(),subjectLabel=ps.map(p=>p.name).join(', ')||'no selected members',stats=ps.length?entityStats(ps):null,metrics=stats?.metrics||[],rows=ranked(metrics),lucky=(lens==='impact'?rows.filter(m=>meaningfulLuckyEligible(m)):rows).slice(0,8),dry=[...rows].sort((a,b)=>a.sort-b.sort).slice(0,8);
