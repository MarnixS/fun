...
function skillIcon(key,{className='skill-icon',alt=''}={}){const src=skillIconUrl(key);return src?`<img class="${escapeHtml(className)}" src="${src}" alt="${escapeHtml(alt)}" loading="lazy" decoding="async">`:''}
function metricLink(kind,key,label=nice(key)){const attr=kind==='skill'?'data-v21-skill':kind==='boss'?'data-v21-boss':'data-v21-metric',icon=kind==='skill'&&key!=='overall'?skillIcon(key):'';return `<button type="button" class="metric-link inline-metric${icon?' metric-link-skill':''}" ${attr}="${escapeHtml(key)}" data-metric-kind="${escapeHtml(kind)}">${icon}<span>${escapeHtml(label)}</span></button>`}
