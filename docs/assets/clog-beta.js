(()=>{
'use strict';
const U=window.UGV21,M=window.UGClogBetaModel,L=window.UGClogLuck;if(!U||!M||!L||document.body.dataset.page!=='clog-beta')return;
...
let luckCache=new Map(),avgLuckCache=new Map();
...
const averageLuck=(id,ps=players())=>{const key=id+'|'+ps.map(p=>p.key).join(',');if(avgLuckCache.has(key))return avgLuckCache.get(key);const rows=ps.map(p=>({player:p,luck:luck(id,[p])})).filter(x=>x.luck&&Number.isFinite(x.luck.percentile));if(!rows.length){avgLuckCache.set(key,null);return null}const percentile=rows.reduce((n,x)=>n+x.luck.percentile,0)/rows.length,pct=percentile*100,text=L.formatPct(pct),band=pct>=90?'lucky':pct<=10?'dry':'normal';const out={percentile,pct,text,band,used:rows.length,total:ps.length,rows};avgLuckCache.set(key,out);return out};
...
function detail(){
 const host=$('#betaItemDetail');if(chosen===null){host.innerHTML='<p>Select an item to see each member’s exact contribution and luck percentile.</p>';return}
 const data=share(chosen),name=model.names.get(chosen)||`Item ${chosen}`,groupLuck=luck(chosen),avg=averageLuck(chosen);
 const calc=groupLuck?`<div class="beta-luck-detail"><div><span>Selected-member average</span><strong class="beta-luck-${avg?.band||'normal'}">${avg?.text||'—'}</strong><small>${avg&&avg.used<avg.total?`${avg.used}/${avg.total} selected members calculable · `:''}50% = ordinary</small></div><div><span>Pooled selected result</span><strong class="beta-luck-${groupLuck.band}">${groupLuck.text}</strong><small>${fmt(groupLuck.observed)} logged · ${fmt(groupLuck.trials)} eligible rolls</small></div></div><div class="beta-luck-basis">${groupLuck.sources.map(s=>`<span>${esc(s.label)}: ${fmt(s.kc)} recorded · ~${s.expected.toFixed(s.expected<10?2:1)} expected</span>`).join('')}${groupLuck.notes.map(n=>`<span class="assumption">${esc(n)}</span>`).join('')}</div>`:'<p class="beta-luck-unavailable">No defensible luck percentile for this selection: the required KC/reward-roll count is not available from the saved WOM data or the item uses an unsupported mechanic.</p>';
 host.innerHTML=`<div class="beta-detail-heading"><img src="https://static.runelite.net/cache/item/icon/${chosen}.png" alt=""><div><h3>${esc(name)}</h3><span>${data.known?fmt(data.total)+' logged across selected members':'No synced data for selected members'}</span></div>${U.itemLink(chosen,'OSRS Wiki ↗')}</div>${calc}<div class="beta-counts">${data.entries.map(p=>{const one=p.count===null?null:luck(chosen,[p]);return `<div style="--member:${p.color}"><span>${esc(p.name)}</span><strong>${p.count===null?'Unknown':fmt(p.count)}</strong><small>${p.count===null?'not synced':data.total?(100*p.share).toFixed(1)+'% of selected drops':'no logged drops'}</small>${one?`<em class="beta-member-luck beta-luck-${one.band}">${one.text} luck</em>`:''}</div>`}).join('')}</div><p class="beta-footnote">Saved Collection Log counts, not current bank ownership. Tiny percentages are mid-distribution luck percentiles for the current member selection: around 50% is ordinary, high values are luckier and low values are drier. Rates are derived from existing RuneLite luck/drop-rate tools; a percentage is only shown where the required roll count can be inferred from WOM. Raid and contribution assumptions are shown above when they apply.</p>`;
}
function render(){
 if(!model)return;luckCache=new Map();avgLuckCache=new Map();
 ...
 $('#betaGrid').innerHTML=rows.length?rows.map(id=>{const data=share(id),name=model.names.get(id)||`Item ${id}`,text=data.known?`${fmt(data.total)} logged`:'unknown',lp=averageLuck(id),lt=lp?` · Average luck of selected members: ${lp.text}${lp.used<lp.total?` (${lp.used}/${lp.total} calculable)`:''}`:'';return `<button type="button" class="beta-item ${data.total?'obtained':'missing'}" data-beta-item="${id}" aria-pressed="${chosen===id}" aria-label="${esc(name)}, ${text}${lp?`, selected-member average luck ${lp.text}`:''}. Show contributions" title="${esc(name)} · ${text}${esc(lt)}"><img src="https://static.runelite.net/cache/item/icon/${id}.png" alt="" loading="lazy">${data.total>1?`<span class="beta-quantity">${data.total>=100000?U.compact(data.total):fmt(data.total)}</span>`:''}${lp?`<span class="beta-luck beta-luck-${lp.band}" aria-label="Average luck ${lp.text}">${lp.text}</span>`:''}${data.total?`<span class="beta-item-pie" aria-hidden="true" style="background:conic-gradient(${pie(data)})"></span>`:''}</button>`}).join(''):'<p class="beta-empty">'+(query?'No matching categories.':'No category data in this saved snapshot.')+'</p>';
