...
function detail(){
 ...
 const data=share(chosen),name=model.names.get(chosen)||`Item ${chosen}`,groupLuck=luck(chosen);
 const calc=groupLuck?`<div class="beta-luck-detail"><div><span>Luck percentile</span><strong class="beta-luck-${groupLuck.band}">${groupLuck.text}</strong><small>50% = ordinary · high = lucky · low = dry</small></div><div><span>Observed / expected</span><strong>${fmt(groupLuck.observed)} / ${groupLuck.expected==null?'—':groupLuck.expected.toFixed(groupLuck.expected<10?2:1)}</strong><small>${fmt(groupLuck.trials)} eligible rolls in this estimate</small></div></div><div class="beta-luck-basis">${groupLuck.sources.map(s=>`<span>${esc(s.label)}: ${fmt(s.kc)} recorded · ~${s.expected.toFixed(s.expected<10?2:1)} expected</span>`).join('')}${groupLuck.notes.map(n=>`<span class="assumption">${esc(n)}</span>`).join('')}</div>`:'<p class="beta-luck-unavailable">No defensible luck percentile for this selection: the required KC/reward-roll count is not available from the saved WOM data or the item uses an unsupported mechanic.</p>';
...
function render(){
 if(!model)return;luckCache=new Map();
 ...
 $('#betaGrid').innerHTML=rows.length?rows.map(id=>{const data=share(id),name=model.names.get(id)||`Item ${id}`,text=data.known?`${fmt(data.total)} logged`:'unknown',lp=luck(id),lt=lp?` · ${L.title(lp)}`:'';return `<button type="button" class="beta-item ${data.total?'obtained':'missing'}" data-beta-item="${id}" aria-pressed="${chosen===id}" aria-label="${esc(name)}, ${text}${lp?`, luck percentile ${lp.text}`:''}. Show contributions" title="${esc(name)} · ${text}${esc(lt)}"><img src="https://static.runelite.net/cache/item/icon/${id}.png" alt="" loading="lazy">${data.total>1?`<span class="beta-quantity">${data.total>=100000?U.compact(data.total):fmt(data.total)}</span>`:''}${lp?`<span class="beta-luck beta-luck-${lp.band}" aria-hidden="true">${lp.text}</span>`:''}${data.total?`<span class="beta-item-pie" aria-hidden="true" style="background:conic-gradient(${pie(data)})"></span>`:''}</button>`}).join(''):'<p class="beta-empty">'+(query?'No matching categories.':'No category data in this saved snapshot.')+'</p>';
